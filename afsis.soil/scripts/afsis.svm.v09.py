...
# remove continuum
for i in range(xtrain.shape[0]):
    xtrain[i,:]=removeContinuum(xtrain[i,:])

for i in range(xtest.shape[0]):
    xtest[i,:]=removeContinuum(xtest[i,:])
...
# !!!!!!!!!!!!!!!!!!!
# have to convert the RC spectra back to RC invers
# !!!!!!!!!!!!!!!!!!!
...
for i in range(xtrain2.shape[0]):
    xtrain2[i,:]=xtrain2[i,:]-xtrain2[i,0]

for i in range(xtest2.shape[0]):
    xtest2[i,:]=xtest2[i,:]-xtest2[i,0]

for i in range(xtrain2.shape[0]):
    xtrain2[i,:]=removeContinuum(xtrain2[i,:])

for i in range(xtest2.shape[0]):
    xtest2[i,:]=removeContinuum(xtest2[i,:])

clf = svm.SVR(C=10000.0, verbose = 1)
for i in range(5):
    idx=[0,1,2,3,4]
    idx.pop(i)
    clf.fit( np.hstack((xtrain2,preds1[:,idx])), labels[:,i])
    preds1[:,i] = clf.predict(np.hstack((xtrain2,preds1[:,idx]))).astype(float)
    preds2[:,i] = clf.predict(np.hstack((xtest2,preds2[:,idx]))).astype(float)
...
sample.to_csv('../submissions/submit.svm.trunc1900.center0.noco2.pass2.v09.csv', index = False)
