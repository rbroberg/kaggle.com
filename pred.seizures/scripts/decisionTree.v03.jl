...
for c in 1:size(cases)[1]
	...
	dct=readdlm(datadir*case*"/dct.csv",',');	
	#xtrain=cc[1:(npre+ninter),:];
	xtrain=hcat(ac[1:(npre+ninter),:],var[1:(npre+ninter),:],ent[1:(npre+ninter),:],cc[1:(npre+ninter),:],dct[1:(npre+ninter),:]);
	ytrain=vcat(ones(npre),zeros(ninter));
	#xtest=cc[(npre+ninter+1):end,:];
	xtest=hcat(ac[(npre+ninter+1):end,:],var[(npre+ninter+1):end,:],ent[(npre+ninter+1):end,:],cc[(npre+ninter+1):end,:],dct[(npre+ninter+1):end,:]);
