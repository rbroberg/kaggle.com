...
version='v2.04' # .90460, .86897, .xxxxxx variance vector along channels, sorted
version='v2.05' # .91517, .88274, .xxxxxx variance slope and intercept
version='v2.06' # .91783, .89700, .xxxxxx variance slope only
version='v2.07' # .90460, .86897, .xxxxxx variance slope and first value
...
def dofitSVMstd(X_train, Y_train, X_test):
    ...
    for j in range(shape[0]):
        a=[np.std(X_train[j][:][i]) for i in range(shape[2])]
        a.sort(reverse=True)
        slope=np.polyfit(a,[i for i in range(len(a))],1)
        b.append([slope[0],a[0]])
    ...
    b=[]
    for j in range(shape[0]):
        a=[np.std(X_test[j][:][i]) for i in range(shape[2])]
        a.sort(reverse=True)
        slope=np.polyfit(a,[i for i in range(len(a))],1)
        b.append([slope[0],a[0]])
