...
version='v2.18' # .94090, .93326, .xxxxx total dwtefd in file, logit
...
def dofitSVMstd(X_train, Y_train, X_test):
    K=6
    #1. join on non-seizures
    noidx=[i for i in range(len(Y_train)) if Y_train[i]==0]
    e=X_train[noidx,:,:]
    e.shape=(e.size,)
    #2. decompose
    cA1, cD1 = pywt.dwt(e, 'db2')
    cA2, cD2 = pywt.dwt(e, 'db2')
    #3.a bin
    Npts=len(cD2) # 102427 for A/Z
    mx=max(cD2)+1 #  163.62055276601635 +1
    mn=min(cD2)-1 # -212.52558883261165 -1
    binpts=int(round(1.*Npts/K))
    #3.b find bins
    ...
    # 4.5
    probs=np.zeros((X_train.shape[0],K))
    for j in range(X_train.shape[0]):
        ps=[]
        e=X_train[j,:,:]
        e.shape=(e.size,)
        eA1, eD1 = pywt.dwt(e, 'db2')
    ...
    dummy=clf.fit(probs, Y_train)
    scores = cross_validation.cross_val_score(clf, probs, Y_train)
    #
    probs=np.zeros((X_test.shape[0],K))
    for j in range(X_test.shape[0]):
        ...
        for k in range(int(K)):
            ps.append(1.*sum((eD2>breaks[k])&(eD2<breaks[k+1]))/len(eD2))
        probs[j,:]=ps
    return [scores,clf.predict(probs)]
