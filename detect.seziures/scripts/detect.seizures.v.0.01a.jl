...
#=
[this_Dstds > Pstd && this_Dents > Pent && this_entrng > Pentrng && this_stdrng[i] > Pstdrng  && this_dstds > Pdstd && this_dents > Pdent
    
=#
    for j in 1:length(szfiles)
        this_ents=getFileEntropies(szfiles[j])
        this_stds=getFileStds(szfiles[j])
        ...
        if (this_Dstds > Pstd && this_Dents > Pent && this_entrng > Pentrng && 
            ...
            print(string(split(szfiles[j],'/')[end],",","1,1\n"))
        else
            print(string(split(szfiles[j],'/')[end],",","0,0\n"))
        end
    end
    for j in 1:length(nofiles)
        this_ents=getFileEntropies(nofiles[j])
        this_stds=getFileStds(nofiles[j])
        this_Dents=sum([i^2 for i in this_ents-mean_noents])
        this_Dstds=sum([i^2 for i in this_stds-mean_nostds])
        this_entrng=this_ents[1]-this_ents[16]
        this_stdrng=this_stds[1]-this_stds[16]
        ...
        if (this_Dstds > Pstd && this_Dents > Pent && this_entrng > Pentrng && 
                this_stdrng > Pstdrng  && this_dstds > Pdstd && this_dents > Pdent)
            print(string(split(nofiles[j],'/')[end],",","1,1\n"))
        else
            print(string(split(nofiles[j],'/')[end],",","0,0\n"))
        end
    end
    for j in 1:nfiles[i]
