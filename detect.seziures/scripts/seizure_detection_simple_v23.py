...
version='v2.22' # .94287, .91812, .xxxxx variance,ent vectors along 16 channels, SVC
version='v2.23' # .94287, .91812, .xxxxx variance,ent vectors along 16 channels, SVR
...
def dofitSVMstd(X_train, Y_train, X_test):
    ...
    #
    x=np.array(b)
    sclg=np.max(x,axis=0)
    x2=x/sclg
    clf=svm.SVC()
    dummy=clf.fit(x2, Y_train)
    scores = cross_validation.cross_val_score(clf, x2, Y_train)
    shape=X_test.shape
    b=[]
    for j in range(shape[0]):
