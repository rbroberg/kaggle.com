...
# -- NOT DOWNSAMPLED --- 
version='v3.30' # .95475, .93620, .xxxxx variance,ent vectors along 16 channels , RandomForestClassifier
version='v3.31' # .95475, .93620, .xxxxx variance,ent mean and std , RandomForestClassifier
...
def dofitSVMstd(X_train, Y_train, X_test):
    div=X_train.shape[1]/2
    x=np.zeros((X_train.shape[0],4))
    x[:,0]=X_train.iloc[:,:div].mean(axis=1)
    x[:,1]=X_train.iloc[:,:div].std(axis=1)
    x[:,2]=X_train.iloc[:,div:].mean(axis=1)
    x[:,3]=X_train.iloc[:,div:].std(axis=1)
    clf=RandomForestClassifier()
    dummy=clf.fit(x, Y_train)
    scores = cross_validation.cross_val_score(clf, x, Y_train)
    div=X_test.shape[1]/2
    x=np.zeros((X_test.shape[0],4))
    x[:,0]=X_test.iloc[:,:div].mean(axis=1)
    x[:,1]=X_test.iloc[:,:div].std(axis=1)
    x[:,2]=X_test.iloc[:,div:].mean(axis=1)
    x[:,3]=X_test.iloc[:,:div].std(axis=1)
